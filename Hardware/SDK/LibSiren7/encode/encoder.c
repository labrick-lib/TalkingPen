             ...
             Word16  *out_words)
{
	...
	Word32  mag_shift_offset;		                                                  //kpit change 30/9
    //Word16 temp;
	Word32 temp;					                                                  //kpit change 30/9
	
	
#ifdef PLOT 
	static int LastEncodeGraph;
#endif	
...
//     }
    ...
    {
        absolute_region_power_index[region] = (absolute_region_power_index[region] + mag_shift_offset);
        move16();
    }
...
#ifdef PLOT
	  if(LastEncodeGraph) DeleteGraphPlot (g722Handle, G722_REGION_BITS, LastEncodeGraph, VAL_IMMEDIATE_DRAW);
	  LastEncodeGraph = PlotWaveform (g722Handle, G722_REGION_BITS, region_mlt_bit_counts, number_of_regions, VAL_SHORT_INTEGER, 1.0, 0.0, 0.0, 1.0,
									  VAL_BASE_ZERO_VERTICAL_BAR, VAL_EMPTY_SQUARE, VAL_SOLID, 1, VAL_GREEN);
                   ...
                   Word16  number_of_bits_per_frame)
{
    ...
    //UWord16 slice;							                               //kpit change 30/9
   ...
   for (region=0; region <= number_of_regions; region++)
    {
        ...
        if (j >= 0)
        {
            temp = extract_l((current_word>>j));
            out_word = (out_word + temp);
        ...
        else
        {
            ...
            //accb = (out_word);
            //acca = (accb + acca);
			acca = ((out_word) + (current_word << j));                                 //kpit change 28/9
            out_word = extract_l(acca);
            
            out_word_bits_free = (out_word_bits_free - current_word_bits_left);
        }
    }
    ...
    for (region=0;region<number_of_regions; region++)
    {
        ...
        //accb = (accb << 4);
        //accb = (accb - number_of_bits_per_frame);
		accb = ((accb << 4) - number_of_bits_per_frame);                               //kpit change 28/9
        test();
        if(accb < 0)        
        {
            temp = (region << 2);
            ...
            //temp = (32 - region_bit_count);
            test();
            //if(temp > 0)
			if((32 - region_bit_count)> 0)	                                          //kpit change 28/9
            ...
            //acca = (acca << 4);
            //acca = (acca - number_of_bits_per_frame);
			acca = ((acca << 4)- number_of_bits_per_frame);	                         //kpit change 28/9
            ...
            while ((region_bit_count > 0) && (acca < 0))
            {
                ...
                temp = (current_word_bits_left - out_word_bits_free);
                test();
                if (temp >= 0)
                {
                    //temp = (32 - out_word_bits_free);
                    //accb = LU_shr(current_word,temp);
                    //accb = (current_word >> temp);
					accb = (current_word >> (32 - out_word_bits_free));	             //kpit change 28/9
                    //slice = (UWord16)extract_l(accb);
                    
                   // out_word = (out_word + slice);
				    out_word = (out_word + ((UWord16)extract_l(accb)));	            //kpit change 30/9
                ...
                else
                {
                    ...
                    test();
                    //accb = slice << temp;
					accb = ((UWord16)extract_l(accb)) << temp;		                    //kpit change 30/9
                ...
                {
                    current_word = *in_word_ptr++;
                    region_bit_count = (region_bit_count - 32);
                ...
                //acca = (acca << 4);
               // acca = (acca - number_of_bits_per_frame);
			   acca = ((out_word_index << 4) - number_of_bits_per_frame);	        //kpit change 28/9
    ...
    {
        test();
        //current_word = 0x0000ffff;				                                //kpit change 28/9
        move32();

        temp = (16 - out_word_bits_free);
        //acca = LU_shr(current_word,temp);
       // acca = (current_word >> temp);
	    acca = (0x0000ffff >> temp);				                                //kpit change 28/9
        //slice = (UWord16)extract_l(acca);

        //out_word = (out_word + slice);
        //out_words[out_word_index++] = out_word;
		//out_words[out_word_index++] =(out_word + slice);                         //kpit change 28/9
		out_words[out_word_index++] =(out_word + ((UWord16)extract_l(acca)));      //kpit change 28/9 30/9
        move16();

        if(out_word != 0) 								                           //kpit change 30/9
		  out_word = 0;
        move16();
        
        if( out_word_bits_free != 16) 						                       //kpit change 30/9
		     out_word_bits_free = 16;
        move16();
        
        acca = (Word16)(out_word_index);
        //acca = (acca<<4);
        //acca = (acca - number_of_bits_per_frame);		                             //kpit change 28/9
		acca = ((acca<<4) - number_of_bits_per_frame);	                           //kpit change 28/9
    }
}
/***************************************************************************
...
***************************************************************************/
void adjust_abs_region_power_index(Word16 *absolute_region_power_index,Word16 *mlt_coefs,Word16 number_of_regions)	//kpit change
{
    Word16 n,i;
    Word16 region;
    Word16 *raw_mlt_ptr;
    ...
    for (region=0; region<number_of_regions; region++)
    {
        ...
        //n = n / 2;
         n = ((absolute_region_power_index[region] - 39)>>1);	                                        //kpit change 28/9
        test();
        if (n > 0)
        {
            temp = region * REGION_SIZE;
           ...
           for (i=0; i<REGION_SIZE; i++)
            {
                ...
                //acca = (*raw_mlt_ptr<<16);				                                             //kpit change 28/9
                //acca = (acca + 32768L);
                //acca = (acca >> n);
                //acca = (acca >> 16);
				 acca = ((((*raw_mlt_ptr<<16)+32768L)>>n)>>16);	                                         //kpit change 28/9
           ...
           // temp = (n << 1);
			...
			temp = (absolute_region_power_index[region] - (n << 1));	                                 //kpit change 28/9
...
Word16 compute_region_powers(register Word16  *mlt_coefs,
                             register Word16  mag_shift,
                             register Word16  *drp_num_bits,
                             UWord16 *drp_code_bits,
                             Word16  *absolute_region_power_index,
                             Word16  number_of_regions)	
{

    register Word16 *input_ptr;
#ifdef _CVI_
    UWord32 long_accumulator;
#else
    ...
    for (region=0; region<number_of_regions; region++)
    {
		...
		/* rms(r) = (1/20 SUM mlt(20r+j)*mlt(20r+j))^0.5 */
		
		// First SUM mlt(20r+j)*mlt(20r+j)
        for (j=0; j<REGION_SIZE; j++)
        {
            itemp1 = *input_ptr++;
            //move16();
            //long_accumulator = L_mac0(long_accumulator,itemp1,itemp1);
            long_accumulator += itemp1 * itemp1;
        }
        ...
        while (acca > 0)
        {
            test();
           // long_accumulator /= 2;
		    long_accumulator = (long_accumulator>>1);                                      //kpit change 28/9
			acca = (long_accumulator & 0x7fff0000L);
            logic32();
		//	power_shift = power_shift + 1;
		power_shift = power_shift++;                                                       //kpit change 30/9
        }
        
        acca = (long_accumulator - 32767);
        
        //temp = (power_shift+15);
        test();
        test();
        logic16();
        while ((acca <= 0) && ((power_shift+15) >= 0))
        {
            test();
            test();
            logic16();
        ...
        //acca = (acca<<1);						                                         //kpit change 30/9
		acca = ((L_deposit_l(mag_shift))<<1);	                                         //kpit change 30/9
        acca = ((power_shift-acca) + 35) - REGION_POWER_TABLE_NUM_NEGATIVES;
        //acca = (35+acca);
        //acca = (acca-REGION_POWER_TABLE_NUM_NEGATIVES);
   ...
   // for (region = temp; region >= 0; region--)
    for (region =(number_of_regions-2) ; region >= 0; region--)	                         //kpit change 28/9
    {
        temp1 = (absolute_region_power_index[region+1]-DRP_DIFF_MAX);
        //temp2 = (absolute_region_power_index[region]-temp1);	                        //kpit change 29/9
        test();
        //if (temp2 < 0)
		if ((absolute_region_power_index[region]-temp1) < 0)
        {
    ...
    //temp2 = (absolute_region_power_index[0]-temp1);
    test();
    //if (temp2 < 0)
	if ((absolute_region_power_index[0]-temp1) < 0)		                                  //kpit change 29/9
    {
    ...
    //temp2 = (absolute_region_power_index[0]- temp1);  	                             //kpit change 29/9
    test();
    //if (temp2 > 0)
	if ((absolute_region_power_index[0]- temp1) > 0)		                             //kpit change 29/9
    {
    ...
    drp_code_bits[0] = (UWord16)(absolute_region_power_index[0]+ESF_ADJUSTMENT_TO_RMS_INDEX);
    move16();
    ...
    for (region=1; region<number_of_regions; region++)
    {
       ...
       // temp2 = (absolute_region_power_index[region]-temp1);
        test();
       // if (temp2 < 0)
	    if ((absolute_region_power_index[region]-temp1) < 0)	                     //kpit change 29/9
        {
            absolute_region_power_index[region] = temp1;
            move16();
        }
    ...
    for (region=1; region<number_of_regions; region++)
    {
        ...
        test();
        //if (temp < 0)
		if ((j-DRP_DIFF_MIN) < 0)	                                                //kpit change 28/9
        {
            j = DRP_DIFF_MIN;
        }
        j = (j-DRP_DIFF_MIN);
        ...
        temp = (temp+DRP_DIFF_MIN);
        absolute_region_power_index[region] = temp;
        move16();

        number_of_bits = (number_of_bits+differential_region_power_bits[region][j]);
                          ...
                          UWord32 *region_mlt_bits)		  
{
	...
	register Word32 temp;		                                                       //kpit chnge 30/9
    ...
    for (*p_categorization_control = 0; *p_categorization_control < temp; (*p_categorization_control)++)
    {
        ...
        move16();
        power_categories[region] = power_categories[region] + 1;
        move16();
    }
    ...
    for (region=0; region<number_of_regions; region++)
    {
        ...
        move16();
        temp = extract_l(region * REGION_SIZE);
        raw_mlt_ptr = &mlt_coefs[temp];	  
        move16();
       // temp = category - (NUM_CATEGORIES-1);
        test();
        //if (temp < 0)
		if ((category - (NUM_CATEGORIES-1)) < 0)	                                //kpit change 28/9
        {
            region_mlt_bit_counts[region] =
            vector_huffman(category, absolute_region_power_index[region],raw_mlt_ptr,
                           &region_mlt_bits[(region<<2)]);
        }
        else
        {
            region_mlt_bit_counts[region] = 0;
        ...
        total_mlt_bits = (total_mlt_bits + region_mlt_bit_counts[region]);
    }


    /* If too few bits... */
    temp = (total_mlt_bits - number_of_available_bits);
    test();
    test();
    logic16();
    ...
    temp1 = (total_mlt_bits - number_of_available_bits);
    temp2 = (*p_categorization_control - (num_categorization_control_possibilities - 1));
    test();
    test();
    logic16();
                      ...
                      UWord32 *word_ptr)
{
    ...
    //Word16  kmax, kmax_plus_one;		                                                 //kpit change 28/9
    Word16  index,signs_index;
    Word8  *bitcount_table_ptr;
    ...
    register Word32 acca;
   // register Word32 accb;				                                                  //kpit change 29/9
   // register Word16 temp;
    register Word32 temp;				                                                  //kpit change 30/9

    register Word16 mytemp;			 /* new variable in Release 1.2 */
    register Word16 myacca;			 /* new variable in Release 1.2 */
   ...
   // num_vecs = number_of_vectors[category];		                                       //kpit change 29/9
    ...
    //kmax = max_bin[category];					                                           //kpit change 28/9
    move16();

    //kmax_plus_one = (kmax + 1);				                                          //kpit change 28/9
    move16();
    ...
    bitcount_table_ptr = (Word8 *)table_of_bitcount_tables[category];
    code_table_ptr = (UWord16 *) table_of_code_tables[category];

    /* compute inverse of step size * standard deviation */
    acca = (step_size_inverse_table[category] * standard_deviation_inverse_table[power_index]);
    //acca = L_shr(acca,1);
    //acca = (acca + 4096);
   // acca = (acca >> 13);
	acca = (acca + 4096) >> 13;							                                //kpit change 28/9
	...
	for (n=0; n<number_of_vectors[category]; n++)		                                //kpit change 29/9
   //for(n=num_vecs;n--;;)
    {
        index = 0;
        move16();
       ...
       // for (j=0; j<vec_dim; j++)
		for (j=0; j<vector_dimension[category];j++)	                                   //kpit change 29/9
        {
            //k = abs_s(*raw_mlt_ptr);
            
            //acca = L_mult(k,inv_of_step_size_times_std_dev);
            //acca = L_shr(acca,1);
		   // acca = (k * inv_of_step_size_times_std_dev);
		    acca = ((abs_s(*raw_mlt_ptr)) * inv_of_step_size_times_std_dev);           //kpit change 29/9
            //acca = L_shr(acca,1);
            ...
            //myacca = (Word16)(k * mytemp);
			myacca = (Word16)((abs_s(*raw_mlt_ptr)) * mytemp);	                      //kpit change 29/9
			myacca = (Word16)(myacca + int_dead_zone_low_bits[category]);
			myacca = (Word16)(myacca >> 2);

            acca = (acca + int_dead_zone[category]);
			...
			//acca = L_shr(acca,13);
            acca = (acca + myacca)>>13;			                                      //kpit change 29/9
			//acca = (acca >> 13);
			...
			
            {
                ...
                //temp = (k - kmax);
				temp = (k - max_bin[category]);	                                     //kpit change 28/9
            ...
            //acca = (index * (kmax_plus_one));		                                //kpit change 28/9
			acca = (index * (max_bin[category]+1));	                                //kpit change 28/9
        ...
        number_of_region_bits = (number_of_region_bits + number_of_code_bits);

        acca = code_bits << number_of_non_zero;
        //accb = L_deposit_l(signs_index);
        //acca = (acca + accb);
		acca = (acca + L_deposit_l(signs_index));	                                  //kpit change 29/9
        code_bits = acca;
        move32();
        ...
        if (j >= 0)
        {
            test();
            acca = code_bits << j;
            current_word = (current_word + acca);
